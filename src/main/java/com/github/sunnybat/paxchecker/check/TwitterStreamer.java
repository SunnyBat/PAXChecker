...
public abstract class TwitterStreamer {
    ...
    /**
     * Creates a new TwitterStream with the given Twitter API. Note that the
     * given Twitter account must be authenticated and ready to use.
     *
     * @param authenticatedTwitter The authenticated Twitter object to use
     */
    public TwitterStreamer(Twitter authenticatedTwitter) {
        this.myTwitter = authenticatedTwitter;
        System.out.println("Twitter initialized! Following: " + usersToCheck.toString());
    }
    ...
    /**
     * Adds a new keyword to filter for. If the Twitter stream is running, this
     * has no effect until restarted.
     *
    ...
    /**
     * Adds a new user to watch. If the Twitter stream is running, this has no
    ...
    /**
     * Starts looking for Twitter updates. If the stream is already started --
     * connected or disconnected -- this method does nothing.
     */
    ...
    /**
     * Checks whether or not the Twitter stream is currently on. This means that
     * it is either connected, attempting to reconnect, or waiting to attempt to
     * reconnect. If false, startStreamingTwitter() may be called to start it.
     *
    ...
    /**
     * Checks whether or not the Twitter stream is currently connected. Note
     * that if the stream is disconnected but is still streaming, it will be
    ...
    /**
     * Enables keyword filtering. This is in addition to filtering by user
     * tweets. Recommended only if you really don't want any false positives.
     */
    ...
    /**
     * Called when a connection to the Twitter API has been made, including
     * reconnects
     */
    public abstract void twitterConnected();

    /**
     * Called when disconnected from the Twitter API and will automatically
     * retry
     */
    public abstract void twitterDisconnected();

    /**
     * Called when disconnected from the Twitter API and will not retry
     * automatically
     */
