...
public interface Setup {
	...
	/**
	 * Prompts the user for settings. Should be called before retrieving any settings (AKA first).
	 * This method will likely block for a long time while the user inputs settings.
	 */
	public void promptForSettings();

	/**
	 * Gets the GmailAccount to use for sending emails. This will be configured such that all that
	 * needs to be called is sendEmail().
	 *
	...
	/**
	 * Gets whether or not to check Twitter for updates. Note that Twitter checking will not
	 * necessarily be usable if this is true.
	 *
	 * @return True to check, false to not
	 */
	...
	/**
	 * Gets the time to wait between checks. Note that this is not usable for Twitter checking, as
	 * Twitter checking is instant.
	 *
	...
	/**
	 * Gets the name of the expo to check. Values returned include West, South, East, Aus. These may
	 * or may not be preceded with "PAX ".
	 *
